...
_CITATION_BRACKET_RE = re.compile(r"\[([^\]]+)\]\s*$") # finds a trailing bracketed citation block at the end of a bullet line:
_CITATION_TAG_RE = re.compile(r"\bC\d+\b")  # Find the citation tags like C1,C2 etc. 

def _extract_bullets(text:str)->List[str]:
    '''Extract the bullets from the text '''
...
def parse_model_output(raw_text:str,evidence_items:Sequence[EvidenceItem],*,min_bullets:int=2,max_bullets:int=4) -> ParsedAnswer:
    parse_warnings:List[str]=[]
    tag_to_chunk_id:Dict[str,str]={}
    allowed_tags=set()

    for it in evidence_items:
        # normalize tags from "[C1]" to "C1"
        tag = it.citation_tag.strip()[1:-1] if it.citation_tag.startswith("[") else it.citation_tag
        allowed_tags.add(tag)

        chunk_id = getattr(it.citation, "chunk_id", "")
...
def parse_model_output(
    ...
    *,
    ...
    return ParsedAnswer(
        mode="answer",
        bullets=bullets,
        citations_by_bullet=citations_by_bullet,
