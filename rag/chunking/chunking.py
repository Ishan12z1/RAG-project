...
_WIKI_ID_RE = re.compile(r"^\s*WikipediaID:\s*(\d+)\s*$", re.IGNORECASE)

def extract_wikipedia_id(normalized_text: str) -> Optional[str]:
    for ln in normalized_text.splitlines():
        m = _WIKI_ID_RE.match(ln.strip())
        if m:
            return m.group(1)
    return None

def html_to_chunkable_markdown(html: str, page_url: str) -> tuple[str, str]:
    ...
    # Pick main content (best-effort selectors; tune if needed)
    main = (
    ...
    # Walk content and render headings/paras/lists
...
# 
def build_spans(normalized_text: str) -> List[Span]:
...
def pack_spans_into_chunks(
    ...
    if corpus_mode == "kilt":
        ...
        for sp in spans:
            s, e = sp.start, sp.end
            txt = normalized_text[s:e]
            tok = tc.count(txt)
            if tok == 0:
                continue
            chunks.append((s, e, sp.section_path, tok))
        return chunks
    chunks: List[Tuple[int, int, str, int]] = []
    n = len(spans)
    i = 0
...
def process_one_doc(
    ...
    packed = pack_spans_into_chunks(normalized, spans, policy, tc,corpus_mode=corpus_mode)

    rows: List[ChunkRow] = []
    wiki_id = extract_wikipedia_id(normalized)
    doc_id_for_rows = wiki_id if (corpus_mode == "kilt" and wiki_id) else doc.doc_id

    for idx, (s, e, section_path, tok) in enumerate(packed):
        chunk_text = normalized[s:e].strip()
        if not chunk_text:
            continue
        # suppress tiny chunks unless it is a heading-only chunk
        if corpus_mode != "kilt":
        ...
        chunk_id, checksum = compute_chunk_id(
            policy.policy_version, doc_id_for_rows, section_path, s, e, chunk_text
        )

        rows.append(
            ChunkRow(
                chunk_id=chunk_id,
                doc_id=doc_id_for_rows,
                source=doc.source,
                title=title,
                url=url,
...
def build_chunks_corpus(
    ...
    for d in iterator:
        try:
            rows = process_one_doc(d, policy, tc,corpus_mode=corpus_mode)
...
def main() -> None:
    args = parse_args()
    # NEW: KILT defaults (Wikipedia evidence tends to be paragraph-sized)
